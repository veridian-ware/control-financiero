...
- `models/db/Tables.kt` — tablas Exposed (`Households`, `Users`, `Categories`, `Transactions`, `Accounts`, `RecurringTransactions`, `RecurringOccurrences`, `Investments`, `Budgets`, `Debts`). `Users` tiene `household_id` nullable; `Categories`/`Transactions`/`RecurringTransactions` tienen `user_id`. `RecurringOccurrences` son los vencimientos (pendiente/pagado) de cada fijo. `Investments` son inversiones (carga manual) por usuario. `Accounts` son cuentas/billeteras; `Transactions` tiene `account_id` nullable (saldo de cuenta = inicial + ingresos − egresos). `Budgets` son límites mensuales de gasto por categoría. `Debts` son cuotas/deudas (carga manual, standalone): préstamos y compras en cuotas (contador X/N) y deudas a personas, con vencimiento manual. ⚠️ **Toda tabla nueva hay que agregarla al `SchemaUtils.createMissingTablesAndColumns(...)` de `DatabaseFactory.init`** o la tabla no se crea (500 "relation does not exist").
- `models/dto/DTOs.kt` + `AuthDTOs.kt` — DTOs serializables de la API.
- `repositories/` — `UserRepository`, `CategoryRepository`, `TransactionRepository`, `RecurringTransactionRepository`, `HouseholdRepository`, `InvestmentRepository`, `AccountRepository`, `BudgetRepository`, `DebtRepository`. Las escrituras filtran por `userId`; las lecturas de transacciones usan el **alcance del hogar** (`HouseholdRepository.memberIds`). `RecurringTransactionRepository` genera los vencimientos del mes por frecuencia (idempotente); al marcar un vencimiento `pagado` crea la `Transaction` real, y al volverlo a `pendiente` la borra.
...
- `ui/screens/` — `DashboardScreen`, `AddTransactionScreen`, `AuthScreen`, `RecurringScreen` (fijos con frecuencia + vencimientos pendiente/pagado), `HouseholdScreen` (hogar compartido), `InvestmentsScreen` (inversiones), `AccountsScreen` (cuentas/billeteras), `BudgetsScreen` (presupuestos), `DebtsScreen` (cuotas y deudas: préstamos/compras en cuotas X/N y deudas a personas, con pagar/deshacer cuota), `ReportsScreen` (gráficos Vico), `SettingsScreen` (perfil), `PremiumScreen` (placeholder). Se navegan desde el **menú lateral** (`ModalNavigationDrawer`); el dashboard tiene la hamburguesa que lo abre. Avatar de iniciales en `ui/components/UserAvatar.kt`.
- `ui/viewmodels/` — `DashboardViewModel`, `AuthViewModel` (login/registro/logout/perfil), `RecurringViewModel`, `HouseholdViewModel`, `InvestmentViewModel`, `AccountViewModel`, `BudgetViewModel`, `DebtViewModel`.
- `work/` — `DebtReminderWorker` (CoroutineWorker que lee `/api/debts` y dispara una **notificación local** de vencimientos) + `DebtReminderScheduler` (programa el `PeriodicWork` diario y crea el canal). Se programa al entrar autenticado desde `MainActivity`, que también pide `POST_NOTIFICATIONS` (Android 13+). En background el token se recupera con `SessionManager.tokenFlow.first()`.
...
| GET    | `/api/debts`                      | Cuotas/deudas (restante, mensual, vencidas/próximas + lista) |
| POST   | `/api/debts`                      | Crear deuda (`description,type,installmentAmount,totalInstallments?,paidInstallments?,creditor?,dueDate?,notes?`) |
| PUT    | `/api/debts/{id}`                 | Editar deuda                                  |
| POST   | `/api/debts/{id}/pay`             | Registrar pago de cuota (avanza contador, empuja vencimiento +1 mes) |
| POST   | `/api/debts/{id}/unpay`           | Revertir el último pago                       |
...
- ✅ Agregado (2026-06-05): **cuotas y deudas** (`debts`, `DebtRepository`, `DebtsScreen`, `/api/debts`). Calca la planilla manuscrita del usuario (bloques DEBO / préstamos-cuotas / gastos): préstamos y compras en cuotas (contador X/N) y deudas a personas, **standalone** (no toca transacciones ni cuentas). Vencimiento manual; `pay` avanza el contador y empuja la fecha **+1 mes** (capeado en N), `unpay` lo revierte; una deuda **simple** (sin N) se salda con un pago. Derivados al vuelo: `remainingAmount`, `progressPct`, `finished`, `dueStatus` (`sin_fecha`/`vencido`/`proximo`≤7d/`ok`) — base para los **recordatorios** (próximo paso). Smoke-test OK (alta 3/6, summary restante/mensual/vencidas/próximas, pay→4/6 +1mes, unpay, tope 6/6, simple saldada, update, delete). ⚠️ Gotcha de siempre: registrar `Debts` en `SchemaUtils.createMissingTablesAndColumns`. ⚠️ Limitación: el `update` solo aplica campos no-nulos, así que desde la edición no se puede *vaciar* fecha/cuotas (sí cambiarlas).
- ✅ Agregado (2026-06-05): **recordatorios de vencimientos** (`work/`, WorkManager + notificaciones locales). Worker diario (`PeriodicWork`, política `KEEP`, 1ª corrida apenas hay red) que lee `/api/debts`, filtra `dueStatus` vencido/próximo y dispara una notificación con la lista. **Solo Android** (no toca el backend; el endpoint ya daba todo). Token en background: el worker hace `SessionManager.tokenFlow.first()` (sincroniza `AuthTokenProvider` que lee el interceptor) → sin sesión no hace nada. Permiso `POST_NOTIFICATIONS` pedido en `MainActivity` al autenticar; canal `debt_reminders`; dependencia `androidx.work:work-runtime-ktx`. ⚠️ Pendiente: probar en dispositivo; por ahora solo cuotas/deudas (no los Fijos); el horario no está fijado a la mañana (corre según el primer enqueue).
